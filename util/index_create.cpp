...
bool IndexCreator::load()
{
	...
	fread(&m_max_ivt, sizeof(int), 1, ivt_fp);
...
{
	int idx_cnt = m_index.size() + 1, ivt_cnt = 0, max_ivt = m_invert_id.size();
	FILE* idx_fp = fopen(m_index_file.c_str(), "wb");
	FILE* ivt_fp = fopen(m_invert_file.c_str(), "wb");
	if(!idx_fp || !ivt_fp)
		return false;
	fwrite(&idx_cnt, sizeof(int), 1, idx_fp);
	fwrite(&max_ivt, sizeof(int), 1, ivt_fp);
	fseek(ivt_fp, sizeof(int), SEEK_CUR);
	for(int i = 0; i < m_index.size(); ++i)
	{
